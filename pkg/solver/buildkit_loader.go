...
import (
	...
	"github.com/getdroplet/packer/pkg/types"
...
func (bkfl *BuildkitFrontendLoader) Load() (*LoadResult, error) {
	...
	bkfl.pkgFile, err = v1alpha1.NewPkgfile(contents)
	if err != nil {
	...
	processPackage := func(baseDir string, contents []byte) error {
		pkg, err2 := v1alpha1.NewPkg(baseDir, "", contents, bkfl.resolveContext(baseDir))
		if err2 != nil {
		...
		log.Printf("loaded pkg %q from %q", pkg.Title, baseDir)
