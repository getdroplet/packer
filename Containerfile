...
FROM ghcr.io/getdroplet/certificates:2022-10-11 AS certificates
FROM ghcr.io/getdroplet/fhs:0.0.1 AS fhs

# runs markdownlint
FROM docker.io/node:19.9.0-alpine3.18 AS lint-markdown
WORKDIR /src
...
ARG GO_BUILDFLAGS
ARG GO_LDFLAGS
ARG VERSION_PKG="github.com/getdroplet/packer/pkg/version"
ARG SHA
ARG TAG
...
ARG VENDOR
ARG TAG
ARG VERSION_PKG="github.com/getdroplet/packer/pkg/constants"
...
RUN FILES="$(goimports -l -local github.com/getdroplet/packer .)" && test -z "${FILES}" || (echo -e "Source code is not formatted with 'goimports -w -local github.com/getdroplet/packer .':\n${FILES}"; exit 1)
...
LABEL org.opencontainers.image.authors="OpenDroplet Authors"
LABEL org.opencontainers.image.documentation="https://github.com/getdroplet/packer/blob/main/README.md"
LABEL org.opencontainers.image.source="https://github.com/getdroplet/packer"
