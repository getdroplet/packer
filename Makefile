...
TOOLCHAIN ?= docker.io/golang:$(GO_VERSION)-alpine3.18
...
GO_BUILDFLAGS += -tags getdroplet.debug
else
GO_LDFLAGS += -s -w
endif
...
fmt:  ## Formats the source code
	@docker run --rm -it -v $(PWD):/src -w /src $(TOOLCHAIN) \
		sh -c "export GO111MODULE=on; export GOPROXY=https://proxy.golang.org; \
		go install mvdan.cc/gofumpt@$(GOFUMPT_VERSION) && \
		go install golang.org/x/tools/cmd/goimports@$(GOIMPORTS_VERSION) && \
		gofumpt -w . && \
        goimports -w -local github.com/getdroplet/packer ."
